...
2. Import vào trình quản lý skill của agent.

### Thủ công (bất kỳ agent nào)

Copy nội dung của [`skills/update-helper/SKILL.md`](skills/update-helper/SKILL.md) vào system prompt hoặc file `AGENTS.md` của bạn.

Sau khi tải, agent sẽ tự động kích hoạt protocol khi gặp các lệnh: `fix this`, `xóa UI cũ`, `refactor`, `port sang chỗ khác`, `patch vừa nãy hỏng rồi`.
