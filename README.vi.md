...
- File có **vài ngàn đến vài chục ngàn dòng** → Đọc cả file là cháy context window ngay.
- Vá dòng 800, sau đó dòng 5,200 thành **dòng 5,203** → Agent vá nhầm chỗ.
- File chứa **ký tự không phải tiếng Anh (tiếng Việt, CJK, Châu Âu, emoji, v.v.)** → Một lệnh `Set-Content` âm thầm xóa BOM → Dữ liệu hỏng, không có cảnh báo.
...
![Agent không có skill: 12 lần tìm kiếm thất bại](images/before-12-searches.png?v=3)

→ Agent tự nhận: *"Hmm, file có vấn đề encoding. Thử cách khác:"*  
→ Công cụ `grep_search` không xử lý được các file khổng lồ UTF-8 BOM chứa ký tự đặc biệt/không phải tiếng Anh.  
...
![Agent với skill: tìm đúng ngay lập tức và trace root cause](images/after-trace-success.png?v=3)

Chi tiết quá trình trace từ error → root cause:

![Quá trình Data Flow Tracing chi tiết](images/comparison-table.png?v=3)
...
![Ước tính tiết kiệm token với Update Helper v4](images/token-savings.png?v=3)
...
| **Encoding-Safe Write** | `.NET ReadAllText/WriteAllText` bảo toàn BOM cho mọi file non-English/UTF-8 |
...
**Agent A** bắt đầu vá một file khổng lồ, tạo session backup, lập bản đồ kiến trúc. **Agent B** tham gia 30 phút sau, không có memory. Không có giao thức → Agent B đọc lại cả file (cháy context), vá nhầm dòng, ghi đè backup của Agent A.
