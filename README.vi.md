...
### 1. Claude Desktop / Web (Skill Manager tích hợp)

1. Tải file [`update-helper.skill`](update-helper.skill) từ repo này.
2. Import trực tiếp vào **Skill Manager** trên Claude Desktop hoặc Claude Web.

### 2. Local AI Agents (Antigravity / OpenClaw)

Các agent này hỗ trợ hệ thống quản lý skill bằng thư mục cục bộ.
...
Copy vào thư mục skills của agent (ví dụ cho Antigravity trên Windows):
...
Copy toàn bộ nội dung của [`skills/update-helper/SKILL.md`](skills/update-helper/SKILL.md) và dán vào file `.cursorrules` hoặc `AGENTS.md` ở thư mục gốc của dự án.

### 4. Thủ công (Bất kỳ nền tảng nào khác)

Dán nội dung của [`skills/update-helper/SKILL.md`](skills/update-helper/SKILL.md) vào system prompt hoặc custom instructions của bạn.
